...
local BASE = {
    crosshair = "PF - Crosshair",
    shape     = "PF - Frame",
    thirds    = "PF - Rule of Thirds",
    diagonals = "PF - Diagonals",
    ellipse   = "PF - Ellipse",
    grid      = "PF - Grid",
    golden    = "PF - Golden Ratio",
}
local SETTINGS_FILE = "lineup_settings.json"
...
local function drawGolden(rectImg, spiralImg, circleImg, sprite, data)
    ...
    local spiralThick = math.max(1, data.goldenSpiralThick)
    local circleThick = math.max(1, data.goldenCircleThick)
    local st          = "Solid"
    local rColor      = data.goldenRectColor
    local sColor      = data.goldenSpiralColor
...
function togglefunctions()
    ...
    for _,l in ipairs(sprite.layers) do
        if l.name:sub(1,5)=="PF - " then
            table.insert(layers,l)
            if not l.isVisible then allVis=false end
        end
    end
    if #layers==0 then app.alert("No LineUp layers found.") return end
    app.transaction("Toggle LineUp Functions",function()
...
function createFunctionsLogic(data,sprite)
    app.transaction("LineUp - Create Functions",function()
        ...
        if data.enableGolden then
            -- Rectangle layer
            local lRect=getLayer(sprite,"PF - Golden Rectangle",true,data.goldenOpacity,data.goldenRectColor)
            ...
            local lSpiral=getLayer(sprite,"PF - Golden Spiral",true,data.goldenOpacity,data.goldenSpiralColor)
            local celSpiral=sprite:newCel(lSpiral,app.activeFrame)
            -- Circle layer
            local imgCircle=Image(sprite.width,sprite.height)
            local lCircle=getLayer(sprite,"PF - Golden Circles",true,data.goldenOpacity,data.goldenCircleColor)
    ...
    if f then
        f:write(JSON.encode({
            ...
            goldenSpiralColor=colorToTable(data.goldenSpiralColor),
            goldenSpiralThick=data.goldenSpiralThick,
...
function createFunctions()
    ...
    local DEF={
        ...
        goldenRectColor=Color{r=255,g=160,b=0,alpha=255},    goldenRectThick=2,
        goldenSpiralColor=Color{r=255,g=60,b=100,alpha=255},  goldenSpiralThick=2,
    ...
    local cfg={
        ...
        goldenSpiralColor=gc("goldenSpiralColor",DEF.goldenSpiralColor),
        goldenSpiralThick=gs("goldenSpiralThick"),
    ...
    local dlg=Dialog{title="LineUp v1.0 - YukiPixels"}
    ...
    dlg:separator{text="  Golden Spiral"}
    dlg:color   {id="goldenSpiralColor", label="Color",              color=cfg.goldenSpiralColor}
    dlg:slider  {id="goldenSpiralThick", label="Thickness",          min=1,max=12, value=cfg.goldenSpiralThick}
    ...
    dlg:button{id="reset",text="  Reset Defaults  ",onclick=function()
        ...
        dlg:modify{id="goldenSpiralColor", color=DEF.goldenSpiralColor}
        dlg:modify{id="goldenSpiralThick", value=DEF.goldenSpiralThick}
    ...
    dlg:button{id="ok",text="  Create  ",onclick=function()
        ...
        createFunctionsLogic({
            ...
            goldenSpiralColor=d.goldenSpiralColor,    goldenSpiralThick=d.goldenSpiralThick,
